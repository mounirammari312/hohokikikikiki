...
export default function Header(){
  ...
  const onSearch = (e:React.FormEvent)=>{ e.preventDefault(); if(q.trim()){ nav(`/shop?q=${encodeURIComponent(q)}`) ; setOpen(false)} }
  useEffect(()=>{
  ...
  return (
    <header className="sticky top-0 z-50 bg-[#FFFCF8]/90 glass border-b border-[#EDE6D8]">
      <div className="bg-[#1A1A1E] text-[#C9A96A] text-[12px] py-2 text-center tracking-widest manrope flex items-center justify-center gap-2 px-3">
        ...
        <span className="md:hidden truncate">{store.announcement}</span>
      ...
      <div className="max-w-[1280px] mx-auto px-4 md:px-6 h-[68px] flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 md:gap-3">
          <form onSubmit={onSearch} className="hidden md:flex items-center bg-white border border-[#EDE6D8] rounded-full px-3 py-1.5 w-[260px] focus-within:border-[#C9A96A] focus-within:ring-2 focus-within:ring-[#C9A96A]/15 transition">
            ...
            <input value={q} onChange={e=>setQ(e.target.value)} placeholder="ابحثي عن قلادة، خاتم..." className="flex-1 outline-none text-sm px-2 bg-transparent placeholder:text-[#B8AA8E]" />
          </form>
          <Link to="/wishlist" className="relative hidden md:flex w-9 h-9 rounded-full border border-[#EDE6D8] items-center justify-center hover:bg-[#FDF2F6] hover:border-[#F6C0D4] transition group" aria-label="wishlist">
          ...
          <Link to="/cart" className="relative w-10 h-10 rounded-full bg-[#1A1A1E] flex items-center justify-center hover:bg-black transition">
      ...
      {open && (
        <div className="md:hidden border-t border-[#EDE6D8] bg-white px-4 py-4 space-y-3">
          <form onSubmit={onSearch} className="flex items-center border border-[#EDE6D8] rounded-full px-3 py-2">
            <Search size={16} className="text-[#9A8A6B]"/><input value={q} onChange={e=>setQ(e.target.value)} placeholder="ابحثي..." className="flex-1 outline-none px-2 text-sm" />
          </form>
