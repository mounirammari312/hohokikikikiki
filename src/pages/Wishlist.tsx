      ...
      <div className="max-w-[1100px] mx-auto px-4 md:px-6 py-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Link to="/shop" className="bg-white border border-[#EDE6D8] px-4 py-2 rounded-full text-sm font-bold flex items-center gap-1"><ArrowLeft size={14}/> المتجر</Link>
        ...
        <div className="grid md:grid-cols-2 gap-4 mt-6">
          {items.map(p=> (
            <div key={p._id} className="bg-white border border-[#EDE6D8] rounded-[20px] p-3 flex gap-3">
              <Link to={`/product/${p._id}`} className="w-28 h-28 rounded-xl overflow-hidden bg-[#FFF8EE] shrink-0"><img src={p.images[0]} alt={p.nameAr} className="w-full h-full object-cover"/></Link>
              <div className="flex-1 min-w-0">
                <Link to={`/product/${p._id}`} className="font-bold text-sm line-clamp-1 hover:text-[#A02A5B]">{p.nameAr}</Link>
